...
class HAN:
    def __init__(self, max_seq_len, max_sent_len, num_classes, vocab_size, embedding_size, max_grad_norm, dropout_keep_proba, learning_rate):
        ...
        # tf graph input
        self.input_x = tf.placeholder(shape=[None, None, None], dtype=tf.int32, name="input_x")
        ...
        with tf.device("/gpu:0"), tf.name_scope("embedding_layer"):
            w = tf.Variable(tf.random_uniform([self.vocab_size, self.embedding_size], -1.0, 1.0), dtype=tf.float32, name="w")  # TODO check if this needs to be marked as untrainable
            self.input_x_embedded = tf.nn.embedding_lookup(w, self.input_x)

        # reshape input_x after embedding
        self.input_x_embedded = tf.reshape(self.input_x_embedded, [self.document_size * self.sentence_size, self.word_size, self.embedding_size])
        ...
        with tf.variable_scope("word_level"):
            self.word_encoder_outputs = self.bidirectional_RNN(num_hidden=self.word_encoder_num_hidden, inputs=self.input_x_embedded)
            word_level_output = self.attention(inputs=self.word_encoder_outputs, output_size=self.word_output_size)

            with tf.variable_scope("dropout"):
                print('self.is_training: {}'.format(self.is_training))
                word_level_output = layers.dropout(word_level_output, keep_prob=self.dropout_keep_proba, is_training=self.is_training)

        # reshape word_level output
        self.sentence_encoder_inputs = tf.reshape(word_level_output, [self.document_size, self.sentence_size, self.word_output_size])
        ...
        with tf.variable_scope("sentence_level"):
            ...
            with tf.variable_scope("dropout"):
                sentence_level_output = layers.dropout(sentence_level_output, keep_prob=self.dropout_keep_proba, is_training=self.is_training)

        # Final model prediction
        with tf.variable_scope("classifier_output"):
            self.logits = layers.fully_connected(sentence_level_output, self.num_classes, activation_fn=None)  # trainable=self.is_training)
    ...
    def bidirectional_RNN(self, num_hidden, inputs):
        ...
        with tf.name_scope("bidirectional_RNN"):
            ...
            ((encoder_fw_outputs, encoder_bw_outputs), (_, _)) = tf.nn.bidirectional_dynamic_rnn(cell_fw=encoder_fw_cell, cell_bw=encoder_bw_cell, inputs=inputs, dtype=tf.float32, time_major=True)
    ...
    def attention(self, inputs, output_size):
        ...
        with tf.variable_scope("attention"):
            attention_context_vector_uw = tf.get_variable(name="attention_context_vector", shape=[output_size],
